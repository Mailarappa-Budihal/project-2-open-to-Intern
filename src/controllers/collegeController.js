...
const createColleges = async function(req, res) {
    try {
        ...
        const regSmall = /^[a-z]+$/;
        if (!regSmall.test(name))
            return res
                .status(400)
                .send({
                    status: false,
                    message: "Please use abbreviations in small letters!!",
                });
        ...
        if (!regFull.test(fullName))
            return res
                .status(400)
                .send({ message: "Please enter FullName in Uppercase & lowercase!!" });
        ...
        res
            ...
            .send({
                status: true,
                message: "College is successfully Created",
                data: collegeCreate,
            });
    } catch (err) {
